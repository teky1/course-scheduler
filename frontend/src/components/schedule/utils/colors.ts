...
export function getGradientColor(
  ...
  for (let i = 0; i < gradient.length - 1; i++) {
    if (
      position >= gradient[i].percent &&
      position <= gradient[i + 1].percent
    ) {
  ...
  const t =
    (position - lowerStop.percent) / (upperStop.percent - lowerStop.percent);
...
export function remapHueEvenly(hue: number): number {
  ...
  const evenHue =
    segment * (360 / colorCount) + withinSegment * (360 / colorCount);
  return evenHue;
}
